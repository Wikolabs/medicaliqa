...
// In docker-compose: BACKEND_URL=http://medicaliqa-backend:8000
// In local dev (next dev outside compose): falls back to localhost
const BACKEND_URL = process.env.BACKEND_URL || "http://localhost:8000";

export async function POST(req: Request) {
  let body: { specialty?: string; question?: string; lang?: "fr" | "en" } = {};
  try {
    body = await req.json();
  } catch {
  ...
  const specialty = typeof body.specialty === "string" ? body.specialty.trim().slice(0, 60) : "";
